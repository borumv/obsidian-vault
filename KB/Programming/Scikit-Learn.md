...
Три главные вещи, которые мы должны сделать:
	1. Разделить данные на features и labels (обычно 'X' & 'y')
...
В Scikit-learn этот этап обычно начинается с создания экземпляра выбранной модели/оценщика с помощью ее конструктора. Затем мы передаем данные для обучения модели, используя метод `.fit()`. После обучения модели мы можем использовать ее для предсказания на новых данных с помощью метода `.predict()`.
Этапы обучения модели и ее использования для предсказания могут повторяться несколько раз, до тех пор, пока не будет достигнуто удовлетворительное качество предсказаний.
...
В Scikit-learn этот этап включает разделение данных на тренировочный и тестовый наборы с помощью метода `train_test_split()`, оценку качества модели на тестовом наборе с помощью метрик, таких как точность (accuracy), F1-мера (F1 score) и коэффициент детерминации (R-squared), а также кросс-валидацию с помощью метода `cross_val_score()`.

Цель этого этапа - выбрать модель, которая дает наилучшее качество предсказания на новых данных. Для этого нужно использовать соответствующие метрики и методы оценки, а также проанализировать результаты для определения проблем, связанных с переобучением или недообучением модели.
